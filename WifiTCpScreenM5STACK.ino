...
void setup() {
  ...
  M5.Lcd.fillScreen(ILI9341_BLACK);              /* Set initial LCD brightness */
...
void SUB_DISPLAY_COURSE ()
{
  ...
  if (gps.course.isValid() == true)                         /* Valid GPS course received ? */
  {
    ...
    if ((OLD_COURSE != INTEGER_VAL) || (COURSE_UPDATE_FLAG == true))
    {
      ...
      M5.Lcd.print(UNITDEG);

      M5.Lcd.setCursor(25, 185);                                /* Clear cardinal info */
      M5.Lcd.print("   ");                                      /* Clear cardinal placeholder */
      M5.Lcd.setCursor(25, 185);                                /* Display cardinal info */
      M5.Lcd.print(gps.cardinal(gps.course.deg()) + String("  "));
    }
  ...
  else                                                        /* Not valid GPS altitude received ? */
  {
    if (COURSE_FAIL_UPDATE == true)                              /* Update needed ? */
    {
      ...
      M5.Lcd.fillRect(4, 145, 100, 40, 0x1E9F);               /* Clear old altitude value */
      M5.Lcd.setCursor(18, 155);                              /* Position cursor */
      M5.Lcd.print("---" + String(UNITDEG));                                 /* Display course placeholder */

      M5.Lcd.setCursor(25, 185);
      M5.Lcd.print("---");   /* Display cardinal placeholder */
    }
...
void SUB_DISPLAY_NO_HOST (bool ping)
{
  ...
  M5.Lcd.setCursor(0, 40);                                /* Display message */
  M5.Lcd.print(F("Connect failed to"));
  M5.Lcd.setTextSize(2);
  M5.Lcd.setCursor(10, 70);
  M5.Lcd.print("" + ip2Str(serverIp) + ":" + String(port));
  M5.Lcd.setCursor(10, 90);
  if (ping) {
    M5.Lcd.print(F("ping server: OK"));
  } else {
    M5.Lcd.print(F("ping server: KO"));
  }
  M5.Lcd.setCursor(5, 120);
  M5.Lcd.print("Wifi connexion valid:");
  M5.Lcd.setCursor(15, 140);
  M5.Lcd.print("SSID: " + String(networkName));
  M5.Lcd.setCursor(15, 160);
  M5.Lcd.print("IP: " + ip2Str(WiFi.localIP()));
  M5.Lcd.setCursor(15, 180);
  M5.Lcd.print("Signal RSSI: " + String(WiFi.RSSI()) + "dBm");
}
...
void SUB_DISPLAY_NO_WIFI (void)
{
  ...
  M5.Lcd.setCursor(0, 80);                                /* Display message */
  M5.Lcd.print(F("Wifi connecting.."));
  M5.Lcd.setTextSize(2);
  M5.Lcd.setCursor(5, 120);
  M5.Lcd.print("SSID: " + String(networkName));
